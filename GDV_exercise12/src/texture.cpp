...
Texture::Texture(std::string_view filename, bool flip, Channels channels, DataType dataType)
{
    ...
    if (dataType == DataType::Float)
        dataPtr = reinterpret_cast<std::byte*>(
            stbi_loadf(filename.data(), &x, &y, &imageChannels, static_cast<int>(channels)));
    else
        dataPtr = reinterpret_cast<std::byte*>(
            stbi_load(filename.data(), &x, &y, &imageChannels, static_cast<int>(channels)));
    if (!dataPtr)
    ...
    *this = {{static_cast<uint32_t>(x), static_cast<uint32_t>(y)},
             channels == Channels::Auto ? Texture::Channels{imageChannels} : channels,
             dataType};

    std::copy_n(dataPtr, dataSize(), data.get());
    stbi_image_free(dataPtr);

    std::cout << "Loaded" << (dataType == DataType::Float ? " HDR" : "")
              << " image file: " << filename << " with " << x << "x" << y << " pixels and "
              << (channels == Channels::Auto ? imageChannels : static_cast<int>(channels)) << "/"

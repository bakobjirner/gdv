    ...
    : ImageView(parent), rayTracer{rayTracer}, cameraControls{cameraControls}
{
    ...
    auto renderMode = new ComboBox(
        rayTracerControls, {"Depth", "Position", "Normal", "Whitted", "Path", "Volumetric"});
...
void RayTracerView::draw(NVGcontext* context)
{
    ...
    if (!texture || m_size != texture->size()) {
        ...
        m_image_shader->set_uniform("useSRGB",
                                    params.mode != RayTracerParameters::RenderMode::Depth
                                        && params.mode != RayTracerParameters::RenderMode::Position
                                        && params.mode != RayTracerParameters::RenderMode::Normal);
    }

...
/// compute the component-wise exponential
inline Point3D exp(const Point3D& a) { return {std::exp(a.x), std::exp(a.y), std::exp(a.z)}; }

/// compute the component-wise power
inline Point3D pow(const Point3D& a, const float exp)
{
    return {std::pow(a.x, exp), std::pow(a.y, exp), std::pow(a.z, exp)};
}

...
public:
    ...
    template <typename T>
    void setVertexAttribute(const std::string_view name, const GLVertexBuffer& buffer)
    {
        ensureActive();
...
public:
    GLTexture(const Texture& image)
    {
        ...
        case Texture::Channels::RGBA:
            ...
            break;
        default:
            throw std::runtime_error("unhandled channel format");
        }
        ...
        case Texture::DataType::UInt8:
            ...
            internalFormat = imageFormat == GL_RGBA ? GL_SRGB_ALPHA : GL_SRGB;
            break;
        case Texture::DataType::Float:
            dataType = GL_FLOAT;
            internalFormat = static_cast<GLint>(imageFormat);
            break;
        }
    ...
    GLTexture(Resolution res, GLenum format = GL_RGBA, GLsizei samples = 1)
        : samples{samples}
    {
        if (res.x * res.y == 0)
            return;
        ...
        if (samples == 1)
            ...
            glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, format, resX, resY, GL_TRUE);
        CHECK_GL();

        if (samples != 1);
            // can't set any sampler values on multisampled textures
        else if (format == GL_DEPTH_COMPONENT) { // shadow map
...
private:
    ...
    /// samples (for multisampled framebuffers)
    GLsizei samples{1};
    ...
    void ensureActive() const
    {
        ...
        if (samples == 1)
            ...
            glGetIntegerv(GL_TEXTURE_BINDING_2D_MULTISAMPLE, reinterpret_cast<GLint*>(&currentTexture));
...
class GLFramebuffer {
public:
    /// create an invalid, unusable framebuffer as a placeholder
    GLFramebuffer() = default;
    /// create a framebuffer with the given resolution
    GLFramebuffer(Resolution res, bool color = true, bool depth = true, GLsizei samples = 1)
        : resolution{res}, colorTarget{color ? res : Resolution{}, GL_RGBA, samples},
          depthTarget{depth ? res : Resolution{}, GL_DEPTH_COMPONENT, samples}
    {
        if (!color && !depth)
        ...
        if (color)
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, samples == 1 ?
        ...
        if (depth)
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, samples == 1 ?
                                       GL_TEXTURE_2D : GL_TEXTURE_2D_MULTISAMPLE,
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE)
            throw std::runtime_error("the framebuffer is incomplete "+std::to_string(status));
    ...
    void blitTo(GLFramebuffer& fb) const {
        glBindFramebuffer(GL_READ_FRAMEBUFFER, framebuffer);
        glBindFramebuffer(GL_DRAW_FRAMEBUFFER, fb.framebuffer);
        GLbitfield mask = 0;
        if (colorTarget.texture && fb.colorTarget.texture)
            mask |= GL_COLOR_BUFFER_BIT;
        if (depthTarget.texture && fb.depthTarget.texture)
            mask |= GL_DEPTH_BUFFER_BIT;
        glBlitFramebuffer(0, 0, static_cast<GLsizei>(resolution.x), static_cast<GLsizei>(resolution.y),
                          0, 0, static_cast<GLsizei>(fb.resolution.x), static_cast<GLsizei>(fb.resolution.y),
                          mask, mask == GL_COLOR_BUFFER_BIT ? GL_LINEAR : GL_NEAREST);
    ...
    const GLTexture& getColorTexture(bool computeMipMaps = true)
    {
        ...
        if (computeMipMaps && colorTarget.samples == 1) {
            colorTarget.bind();
...
private:
    ...
    void swap(GLFramebuffer&& other) {
        ...
        std::swap(colorTarget, other.colorTarget);

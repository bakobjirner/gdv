...
{
    return {std::exp(c.r), std::exp(c.g), std::exp(c.b), std::exp(c.a)};
}

inline Color pow(const Color& c, const float exp)
{
    return {std::pow(c.r, exp), std::pow(c.g, exp), std::pow(c.b, exp), std::pow(c.a, exp)};
}

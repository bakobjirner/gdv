...
struct Texture {
    ...
    enum class Channels { Auto = 0, R = 1, RG = 2, RGB = 3, RGBA = 4 };
    ...
    Texture(std::string_view filename, bool flip, Channels channels, DataType dataType);
    /// load a texture from a file using default parameters
    Texture(std::string_view filename, bool flip = true)
        : Texture{filename, flip, Channels::Auto,
                  filename.ends_with(".hdr") ? DataType::Float : DataType::UInt8}
    {
    }

...
public:
    ...
    const std::vector<std::pair<uint32_t, uint32_t>>& getSmoothGroups() const
    {
        return smoothGroups;
    }
    ...
    Triangle getTriangleFromFaceIndex(uint32_t i) const { return getTriangleFromFace(faces.at(i)); }
    /// get the triangle normals that correspond to the given face index
    TriangleNormals getTriangleNormalsFromFaceIndex(uint32_t i) const
    {

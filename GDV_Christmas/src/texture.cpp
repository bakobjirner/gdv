...
Texture::Texture(std::string_view filename, bool flip)
{
    ...
    const bool hdr = filename.ends_with(".hdr");

    std::byte* dataPtr{nullptr};
    if (hdr)
        dataPtr =
            reinterpret_cast<std::byte*>(stbi_loadf(filename.data(), &x, &y, &imageChannels, 0));
    else
        dataPtr =
            reinterpret_cast<std::byte*>(stbi_load(filename.data(), &x, &y, &imageChannels, 0));
    if (!dataPtr)
    ...
    *this = {{static_cast<uint32_t>(x), static_cast<uint32_t>(y)},
             ...
             hdr ? Texture::DataType::Float : Texture::DataType::UInt8};

    std::copy_n(dataPtr, dataSize(), data.get());
    stbi_image_free(dataPtr);

    std::cout << "Loaded" << (hdr ? " HDR" : "") << " image file: " << filename << " with " << x
              << "x" << y << " pixels and " << imageChannels << " channel(s)." << std::endl;
}
